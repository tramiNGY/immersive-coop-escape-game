...
public class LanternBlueLight : MonoBehaviour
{
    ...
    [SerializeField] private RunesAppear runesFade;
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other == CompassCollider)
        {
            ...
            StartCoroutine(DelayedLantern());
        }
    }


    IEnumerator DelayedLantern() // coroutine to delay Runes appear after lantern light triggered
    {
        yield return new WaitForSeconds(5f); // delay time
        runesFade.gameObject.SetActive(true); // Make Rune appear (enable)
    }
}
